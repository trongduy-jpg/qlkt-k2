...
function buildMovementRow(
  ...
  return {
    ...
    issue_date: order.issueDate || order.occurredDate || null,
    issue_sku: order.issueSku || order.itemSku || order.sku || null,
    issue_product_name: order.issueProductName || order.productName || null,
    issue_qty_piece: order.issueQtyPiece || order.qtyPiece || null,
    return_date: order.returnDate || order.occurredDate || null,
    return_sku: order.returnSku || order.itemSku || order.sku || null,
    return_product_name: order.returnProductName || order.productName || null,
    return_qty_piece: order.returnQtyPiece || order.qtyPiece || null,
...
function mergeMovementResult(order: ProductionOrder, data: unknown): ProductionOrder {
  const mapped = movementRowToProductionOrder(data as unknown as MovementRow);
  return {
    ...order,
    ...mapped,
    code: order.code,
    ...
    issueDate: order.issueDate || mapped.issueDate,
    issueSku: order.issueSku || mapped.issueSku,
    issueProductName: order.issueProductName || mapped.issueProductName,
    issueQtyPiece: order.issueQtyPiece ?? mapped.issueQtyPiece,
    returnDate: order.returnDate || mapped.returnDate,
    returnSku: order.returnSku || mapped.returnSku,
    returnProductName: order.returnProductName || mapped.returnProductName,
    returnQtyPiece: order.returnQtyPiece ?? mapped.returnQtyPiece,
