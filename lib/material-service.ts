...
export async function loadProductionOrders(): Promise<ProductionOrder[]> {
  ...
  return (data as unknown as MovementRow[]).map(movementRowToProductionOrder);
}
    ...
    .upsert(
      {
        ...
        quantity_piece: order.qtyPiece || null,
        planned_date: order.occurredDate || null,
        planned_stage: order.stage || null,
        planned_worker: order.worker || null,
        planned_material: order.material || null,
        planned_gold_age: order.goldAge || null,
        planned_material_type: order.materialType || null,
        issued_gram: order.issued || null,
        returned_gram: order.returned || null,
        powder_gram: order.powder || null,
